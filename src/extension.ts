...
export function
  ...
  let disposable = vscode.commands.registerCommand('extension.jekyllPost',
    async (uri: vscode.Uri) => {
      ...
      getFileNameFromUser().then(async res => {
        const userFileName = res;
        const userFilePath = addDateToFilename(userFileName);
        try {
          ...
          if (shouldInsertFrontMatter()) {
            ...
            const snippetStr = postSnippet.replace('newFileName', userFileName);
